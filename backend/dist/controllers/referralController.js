...
exports.updateReferralStage = updateReferralStage;
...
async function getAllReferrals(req, res) {
    try {
        ...
        const where = {};
        if (originFacilityId)
            where.originFacilityId = String(originFacilityId);
        if (destinationFacilityId)
            where.destinationFacilityId = String(destinationFacilityId);
        if (status)
...
async function createReferral(req, res) {
    try {
        const { patientId, originFacilityId, destinationFacilityId, reason, clinicalSummary, priority } = req.body;
...
async function updateReferralStage(req, res) {
    try {
        const { id } = req.params;
