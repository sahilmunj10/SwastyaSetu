...
const aiFollowupRisk_1 = require("../services/aiFollowupRisk");
...
async function getFollowUps(req, res) {
    try {
        ...
        const where = {};
        if (assignedWorkerId)
            where.assignedWorkerId = String(assignedWorkerId);
        if (status)
...
async function createFollowUp(req, res) {
    try {
        const { patientId, assignedWorkerId, category, description, dueDate, priority, notes } = req.body;
        ...
        const isAbnormalVital = !!lastVital && ((lastVital.systolicBp || 0) >= 140 ||
            (lastVital.diastolicBp || 0) >= 90 ||
            (lastVital.bloodGlucose || 0) >= 180);
        const aiRisk = (0, aiFollowupRisk_1.calculateFollowupRisk)({
